---
title: "fish shell を使うべき理由と、おすすめの設定・プラグイン"
date: "2026-04-02"
...
fish はこれらの機能が **すべてデフォルトで組み込まれています**。
インストールした瞬間から、以下が何も設定しなくても動きます。
...
2024 年末にリリースされた fish 4.0 は、長年 C++ で書かれていたコードベースが **Rust で完全に書き換え** られました。
これにより、以下の恩恵があります。
...
fish は、man ページを解析して自動的に補完候補を生成する `fish_update_completions` というコマンドを持っています。
これにより、インストール済みのほとんどの CLI ツールに対して、何もしなくても基本的な補完が効きます。
...
また、多くのモダンな CLI ツールは、fish 向けの補完スクリプトを自動生成する機能を持っています。
例えば、以下のツールの補完を生成して `~/.config/fish/completions/` に配置しています。
...
例えば `git commit --` と入力して `Tab` を押すと、`--amend`、`--message` などのフラグが、それぞれの説明文付きで一覧表示されます。

zsh でも似たことは可能ですが、`zsh-completions` のインストールと `.zshrc` への設定追加が必要です。
fish ではこれがデフォルトの挙動です。
...
zsh で `.zshrc` を育てるのに疲れた方、あるいは特にカスタマイズせずに快適なシェル環境を手に入れたい方は、ぜひ fish を試してみてください。
