...
これにより、手元での繰り返し実行では「変更していない部分のビルドを丸ごとスキップする」といった最適化が、特別な仕組みなしで実現できます。なお、スキップ判定はファイルの更新時刻や前回実行の記録に基づくため、毎回まっさらな環境から始まる CI で効かせるには、成果物のキャッシュを別途用意する必要があります。
