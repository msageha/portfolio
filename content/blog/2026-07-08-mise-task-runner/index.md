...
ただし公式ドキュメントは、`run` を省略した書き方をスクリプトや CI には使わないよう案内しています。将来 mise 本体が同じ名前のサブコマンドを追加すると、タスクの方が隠れてしまうからです。対話的にさっと叩く分には問題ありませんが、スクリプト化するなら `mise run build` と書いたほうが安全です。
...
Makefile でターゲットの依存関係を書いていたのと同じことができます。
...
これにより、手元での繰り返し実行では「変更していない部分のビルドを丸ごとスキップする」といった最適化が、特別な仕組みなしで実現できます。判定の中身は単純で、出力ファイルの中でもっとも古いものの更新時刻と、ソースファイルの中でもっとも新しいものの更新時刻を比較しているだけです。実行履歴のようなものは持たないので、毎回まっさらなチェックアウトから始まる CI ではソースと出力の両方が新規に生成されてしまい、この比較がそもそも働きません。CI で効かせるには、成果物を跨ジョブでキャッシュする仕組みを別途用意する必要があります。

### 引数を受け取るタスク

タスクは [usage](https://usage.jdx.dev/) 記法（mise と同じ作者が策定している、CLI の引数やフラグを宣言するための仕様）で引数やフラグを宣言できます。宣言した引数はテンプレート `{{usage.xxx}}` や環境変数 `$usage_xxx` で参照でき、依存タスクへ引き渡すこともできます。
...
長いスクリプトは、`mise-tasks/` ディレクトリに実行可能ファイルとして置くだけでタスクになります（file tasks）。ファイルには実行権限が必要で、`chmod +x` を忘れるとタスクとして認識されない点だけ注意してください。ファイル名がタスク名になり、シェバン付きなら言語も自由です。シェバンとは、スクリプトの 1 行目に書く `#!/usr/bin/env bash` のような `#!` で始まる行のことで、そのファイルをどのインタープリタで実行するかを指定します。これを `#!/usr/bin/env python3` に変えれば Python で、`#!/usr/bin/env node` に変えれば JavaScript でタスクを書けます。
...
タスク名を省略した場合の挙動はドキュメント上明記されていないため、確実に動かすなら `mise watch <task>` の形で使うのがおすすめです。各言語の watch ツール（nodemon、cargo-watch など）を個別に入れなくても、タスク定義さえあれば同じ体験が得られます。

---

## mise generate：設定ファイルの自動生成

README の「開発の始め方」欄が実態とズレていた経験は、たいていの開発者にあるはずです。タスクを一つ追加したのに、コマンド一覧を書いた README の更新だけ忘れる。よくある話です。

`mise generate`（エイリアス `mise gen`）は、mise の設定から周辺ツール向けのファイルを生成するコマンド群です。使い方次第で、この「書き忘れ」を構造的に防げます。
...
もっとも、これだけでは冒頭の「書き忘れ」は解決しません。生成コマンドを手で叩く運用のままでは、タスクを追加したのに再生成を忘れて README が古くなる、という乖離はいつまでも起こり得ます。

ここで効いてくるのが `git-pre-commit` との連携です。`mise generate git-pre-commit` はデフォルトで「`pre-commit` という名前のタスクを実行する」git フックを生成するので、その中に task-docs の再生成を含めておけば、コミットのたびにドキュメントが自動更新されます。
...
続きは[第 4 弾](/blog/mise-env-management/)、シリーズ最終回です。環境変数まわりを仕上げて締めくくります。
