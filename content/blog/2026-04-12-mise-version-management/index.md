...
uv には `uv python install 3.13` というコマンドがあり、Python インタープリタ自体のインストールも行えます。ここで「mise でも Python を管理しているなら、ダブル管理になるのでは？」という疑問は正当です。
...
**推奨する回避策: Python バージョン管理は mise に統一し、`uv python install` は使わない**
...
- **uv**: Python のパッケージ管理・仮想環境管理（`uv python install` は使わない）
...
uv プロジェクト（`uv.lock` がある場合）では、mise の `python.uv_venv_auto` 設定を使うと、ディレクトリに入ったときに仮想環境を自動で有効化できます。
