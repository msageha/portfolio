...
ここまでの設定は `config:recommended` を土台にしたものだが、Renovate のメンテナ自身は、これをさらに一歩進めた `config:best-practices` という preset を勧めている。中身は `config:recommended` に、次の preset を extends したものである。
