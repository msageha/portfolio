...
この発想自体は目新しいものではない。[Renovate](https://github.com/renovatebot/renovate) はずっと前から `minimumReleaseAge` という名前で同じことをやっている。GitHub がデフォルト機能にまで格上げしたということは、更新のタイミングを遅らせること自体が、もう趣味の設定ではなく標準的な防御手段だと業界が認めたということでもある。

このリポジトリでも Renovate を使っている。設定は、所属する組織の共通テンプレート [dope-corp/template](https://github.com/dope-corp/template) の renovate.json をベースに、mise 連携まわりをこのリポジトリ用に足したものだ。実際の設定に沿って、何を、なぜ自動化しているかを見ていく。
...
Dependabot との一番の違いは、設定の組み方にある。Renovate の設定は `extends` でプリセットを積み重ねる形式で、`config:recommended` のような公式プリセットも、コミュニティ製のプリセットも、自分で書いた `packageRules` も同じ書式で扱える。後半で扱う `customManagers` を使えば、Renovate がもともと知らないファイル形式にまで追従の対象を広げられる。この自由度が、mise 連携や独自ファイルへの追従を可能にしている。
...
`pre-commit` の `enabled: true` は、実は一手間かけている行である。Renovate の pre-commit マネージャー（`.pre-commit-config.yaml` の `rev:` を追従させる機能）はデフォルトで無効になっている。何もしなければ、pre-commit hooks のバージョンだけがひっそりと古いまま取り残される。明示的に有効化して初めて、他の依存関係と同じ土俵に乗る。
