...
patch/minor の更新は、`minimumReleaseAge` を満たし、かつ CI が green であれば自動でマージされる。ここで使われている `automergeType: "pr"` は Renovate のデフォルト値で、通常どおり PR を作成し、CI が通ってからその PR をマージする。何がマージされたのかは、あとから PR の履歴で誰でも追える。
...
Renovate が標準で解釈できるのは、`package.json` や `go.mod` のような、エコシステムごとに決まったマニフェスト形式だけである。それ以外の独自な書き方には、標準のマネージャーは反応しない。この隙間を埋めるのが `customManagers` で、正規表現でファイル中からバージョン情報を抜き出し、既存の datasource に照合させる仕組みである。
...
Renovate が優れているのは、対応エコシステムの数ではない。依存関係の更新という一つの経路に対して、猶予期間・automerge・脆弱性アラートの扱いから、独自ファイルへの追従や mise.toml との連携まで、自分の判断で層を積み重ねられる自由度にある。

コーディングエージェントに依存関係の更新作業そのものを任せる機会が増えるほど、Renovate が出す更新 PR という経路をどれだけ固めているかが、そのまま防御力になる。`minimumReleaseAge` はエージェントが新しく叩く install コマンドまでは守れないが、日常的に繰り返されるこの経路を機械的に固めてくれる。エージェントに開発環境を任せるなら、まず手を付けておく価値のある設定だと思う。
